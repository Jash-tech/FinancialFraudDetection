...
from financial_fraud_detection.constants import APP_HOST, APP_PORT
from financial_fraud_detection.pipeline.prediction_pipeline import FraudData, FraudClassifier
from financial_fraud_detection.pipeline.training_pipeline import TrainPipeline
...
# DataForm class to handle the form submission
class DataForm:
    ...
    # Fetch the data from the form submitted via POST request
    async def get_fraud_data(self):
...
async def index(request: Request):
    return templates.TemplateResponse("fraud.html", {"request": request, "context": "Rendering"})


# Train route, which will trigger the training pipeline
@app.get("/train")
...
async def predictRouteClient(request: Request):
    try:
        ...
        await form.get_fraud_data()

        # Map the form data to the model input
        ...
        fraud_df = fraud_data.get_fraud_input_data_frame()

        # Load the trained model and make the prediction
        model_predictor = FraudClassifier()
        prediction = model_predictor.predict(dataframe=fraud_df)[0]
        ...
        return templates.TemplateResponse("fraud.html", {"request": request, "context": status})
